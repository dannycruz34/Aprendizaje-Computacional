...
datos = read.table("/Users/DanniC/Desktop/Datasets/Hepatitis/hepatitis.txt",header = F)
datos2 = datos[,2:20]
...
for(i in 1:nrow(datos)){
  datos[i,21] <- paste(toString(CC_euc$vectorFinal[i]))
}

ari <- adjustedRandIndex(unlist(datos[,1]),unlist(datos[,21]))
ari
